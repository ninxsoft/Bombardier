...
struct Sidebar_Previews: PreviewProvider {
  static var previews: some View {
    Sidebar(displayType: .constant(.models),
            models: [.example],
...
struct Sidebar: View {
  ...
  private func filteredModels() -> [Model] {
    ...
    if filter {
      filteredModels = models.filter({ $0.family == family })
      ...
      for type in Model.Family.allCases {
        var filteredFamily: [Model] = models.filter({ $0.family == type })
  ...
  private func filteredPackages() -> [Package] {
    ...
    if sort {
      filteredPackages = filteredPackages.sorted(by: { ascending ? $0.date < $1.date : $0.date > $1.date })
    }
